# Modified from https://github.com/open-mmlab/mmcv/blob/master/mmcv/utils/config.py
# Allows modular heirarchical .py config files
...
class Config:
    ...
    def _file2dict(filename, use_predefined_variables=True):
        ...
        if fileExtname not in [".py"]:
            raise OSError("Only py type are supported!")
        ...
        with tempfile.TemporaryDirectory() as temp_config_dir:
            ...
            if filename.endswith(".py"):
                ...
                raise Exception(
                    "YAML/JSON is not currently supported. .py config files are better anyway, why not use it??? "
                )
            # close temp file
            temp_config_file.close()

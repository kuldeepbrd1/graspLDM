...
def create_gripper_marker(color=[0, 0, 255], tube_radius=0.001, sections=6):
    """Create a 3D mesh visualizing a parallel yaw gripper (four cylinders).
...
def visualize_pc_grasps(
    ...
    if c is not None:
        ...
        gripper_marker = [create_gripper_marker(color=[0, 255, 0, 255])] * grasps.shape[0]

    gripper_markers = [
        gripper_marker[i].copy().apply_transform(t) for i, t in enumerate(grasps)
    ]
    return trimesh.Scene([trimesh.points.PointCloud(pc, colors=pc_colors)] + gripper_markers)
